...
import type { ResearchFeedback, ResearchPlan, ResearchSearch } from "./types";
...
export async function createResearchPlan({
  ...
  while (attempts < maxAttempts) {
    ...
    try {
            ...
            },
            {
              ...
              content: JSON.stringify({
                task: previousFailure
                  ? "Repair the previous research-plan failure and return valid JSON."
                  : "Generate API and web-search queries that can reveal concrete developer pain.",
                previousFailure,
                originalQuery: query,
                selectedSources,
                openWebTargets,
      ...
      return { plan, callsAttempted: attempts, mode: "used" };
    } catch (error) {
      lastError = error;
    }
  }
  ...
  return {
    ...
    mode: "fallback",
    failureReason: `Research planning failed after ${attempts} attempts: ${lastError}`,
  };
}
...
export async function createGapExpansionSearches({
  ...
  while (attempts < maxAttempts) {
    ...
    try {
        ...
        withReasoningEffort({
          ...
          messages: [
            ...
            },
            {
              ...
              content: JSON.stringify({
                task: previousFailure
                  ? "Repair the previous gap-expansion failure and return valid JSON."
                  : "Find missing research angles and return only new searches worth running.",
                previousFailure,
                originalQuery: query,
                selectedSources,
                openWebTargets,
                ...
                researchFeedback: buildGapExpansionFeedbackPayload(researchFeedback),
                constraints: [
                  "Do not repeat an executed search.",
                  "Prefer sources with little or no accepted evidence so far.",
                  "Use page-triage feedback to understand why earlier searches failed.",
                  "Address named failure modes directly in the next search wording.",
                  "Prefer page-triage followUpSearches when they are specific and not duplicates.",
                  "If previous results drifted semantically, add disambiguating terms such as automation, headless, Playwright, scraping, or agent navigation.",
                  "If previous pages were marketing pages or homepages, prefer forum, issue, workaround, incident, bug, or debugging wording.",
                  "If Reddit pages were login-gated, avoid adding more Reddit searches unless explicit subreddit targets were provided.",
      ...
      return { searches, callsAttempted: attempts, mode: "used" };
    } catch (error) {
      lastError = error;
    }
  }
  ...
  return {
    ...
    callsAttempted: attempts,
    mode: "fallback",
    failureReason: `Gap expansion failed after ${attempts} attempts: ${lastError}`,
  };
}

function buildGapExpansionFeedbackPayload(feedback: ResearchFeedback[]) {
  const latest = feedback.slice(-2);
  const rejectedPages = latest
    .flatMap((packet) => packet.rejectedPages)
    .slice(0, 8);
  const suggestedSearches = unique(
    latest.flatMap((packet) => packet.suggestedSearches)
  ).slice(0, 8);

  return {
    summary: latest.flatMap((packet) => packet.summary).slice(0, 8),
    rejectedPages,
    suggestedSearches,
  };
}
